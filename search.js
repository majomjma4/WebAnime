(() => {
  ...
  const hydrateCardsWithResults = (items, mediaType, query = "") => {
    ...
    cards.forEach((card, idx) => {
      ...
      card.setAttribute("data-type", pageForType(mediaType) === "peliculas.html" ? "Pelicula" : "Anime");
  ...
  const pageForType = (mediaType) => {
    ...
    if (t.includes("movie") || t.includes("pel?cula")) return "peliculas.html";
    return "series.html";
  };
  ...
  const bindInput = (input) => {
    ...
    const fetchApiSuggestions = async (term) => {
      ...
      try {
        const res = await fetch(`${API_BASE}?q=${encodeURIComponent(q)}&limit=${API_SUGGEST_LIMIT}&order_by=popularity&sort=asc`);
        if (!res.ok) return [];
        const json = await res.json();
