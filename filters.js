(function () {
  ...
  function setup() {
    ...
    const hideCardYears = /series\.html|peliculas\.html/i.test(window.location.pathname || "");
    ...
    const renderItems = (items) => {
      ...
      items.slice(0, 5).forEach((item, idx) => {
        ...
        row.addEventListener("click", () => {
          ...
          window.location.href = id ? `detail.html?mal_id=${id}&q=${q}` : `detail.html?q=${q}`;
        });
  ...
  function injectTestCards(isMoviesPage) {
    ...
    const createCard = (item, overrides) => {
      ...
      const hrefId = item.mal_id
        ? `detail.html?mal_id=${encodeURIComponent(String(item.mal_id))}&q=${encodeURIComponent(title)}`
        : `detail.html?q=${encodeURIComponent(title)}`;
    ...
    if (!isMoviesPage) {
      const tasks = [
        ...
        { url: "https://api.jikan.moe/v4/anime?status=airing&type=tv&order_by=score&sort=desc&limit=1", type: "Serie de TV", status: "En emisión" },
        { url: "https://api.jikan.moe/v4/anime?status=upcoming&type=tv&order_by=score&sort=desc&limit=1", type: "Serie de TV", status: "Próximamente" },
    ...
    const movieAssignments = [
      ...
      { status: "Próximamente", type: "Basada en serie" },
