        ...
        <aside class="lg:col-span-2">
            ...
            <div class="space-y-6">
              <div class="space-y-2">
                <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant"
                  for="filter-search">Buscar</label>
                <div class="flex items-center gap-2 rounded-xl bg-surface-container-high px-4 py-3">
                  <span class="material-symbols-outlined text-primary">search</span>
                  <input id="catalog-search-input" name="catalog_search_input" data-catalog-search="1"
                    class="w-full bg-transparent border-none focus:ring-0" type="search" placeholder="Ej: Frieren"
                    aria-label="Buscar anime" autocomplete="off" autocorrect="off" autocapitalize="off"
                    spellcheck="false" />
                </div>
              </div>
              <div class="space-y-2">
                <label
                  class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant">G&eacute;neros</label>
              ...
              <div class="space-y-4">
                <div class="space-y-2">
                  <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant"
                    for="filter-year">A&ntilde;o</label>
                  <select id="filter-year" class="w-full rounded-xl bg-surface-container-high px-4 py-3 text-on-surface"
                    aria-label="Filtrar por a&ntilde;o">
                    <option>Todos</option>
                    <option>2026</option>
                    <option>2023</option>
                    <option>2022</option>
                ...
                <div class="space-y-2">
                  <label class="block text-xs font-bold uppercase tracking-widest text-on-surface-variant"
                    for="filter-type">Tipo</label>
                  <select id="filter-type" class="w-full rounded-xl bg-surface-container-high px-4 py-3 text-on-surface"
              ...
              <div class="flex gap-3">
                <button class="flex-1 rounded-full bg-surface-container-high py-3 text-sm font-semibold text-on-surface"
                  type="button">Reiniciar</button>
                <button class="flex-1 rounded-full bg-primary py-3 text-sm font-bold text-on-primary"
          ...
          <section class="mt-6 rounded-lg bg-surface-container-low p-4" aria-label="Ranking">
            <div class="mb-3 flex items-center justify-between gap-3">
              ...
              <a class="text-xs font-semibold text-primary hover:text-primary-dim"
                href="<?= route_path('ranking') ?>">Ver ranking</a>
            </div>
        ...
        <section class="lg:col-span-10" aria-label="Resultados de cat&aacute;logo">
          <header class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex items-center gap-3 text-sm text-on-surface-variant">
              ...
              <button class="flex items-center gap-1 font-bold text-on-surface" type="button"
          ...
          <section class="space-y-6" aria-live="polite">
            ...
            <div class="hidden" data-state="empty">
              <div class="rounded-lg bg-surface-container-low p-10 text-center">
                ...
                <p class="mt-2 text-on-surface-variant">Prueba otros filtros para ver m&aacute;s t&iacute;tulos
                  disponibles.</p>
              </div>
            </div>
            ...
            <div class="hidden" data-state="error">
              <div class="rounded-lg bg-error-container/30 p-10 text-center">
                ...
                <p class="mt-2 text-on-error-container/80">No pudimos cargar el cat&aacute;logo. Int&eacute;ntalo
                  nuevamente</p>
                <button class="mt-6 rounded-full bg-primary px-6 py-3 text-sm font-bold text-on-primary"
          ...
          <!-- AnimeCard Component -->
          <section
            class="mt-[clamp(0.6rem,0.9vw,0.8rem)] grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6"
            aria-label="Grid de anime">
            ...
            if (!empty($animes)) {
              foreach ($animes as $a) {
                ...
                ?>
                <article
                  class="group rounded-lg bg-surface-container-low p-4 transition-transform duration-300 ease-snappy hover:scale-[1.02]"
                  data-anime-card data-title="<?= htmlspecialchars(strtolower($a['titulo'])) ?>"
                  data-genres="<?= htmlspecialchars($generos_str) ?>" data-year="<?= htmlspecialchars($a['anio'] ?? '') ?>"
                  data-type="<?= htmlspecialchars($a['tipo']) ?>"
                  data-status="<?= htmlspecialchars($a['estado'] ?? 'Desconocido') ?>"
                  data-mal-id="<?= htmlspecialchars($a['mal_id'] ?? '') ?>">
                  <a class="block"
                    href="<?= detail_path((string) ($a['mal_id'] ?? ''), (string) $a['titulo'], (string) ($a['id'] ?? '')) ?>"
                    aria-label="<?= htmlspecialchars($a['titulo']) ?>"
                    data-mal-id="<?= htmlspecialchars((string) ($a['mal_id'] ?? $a['id'])) ?>">
                    <div class="relative aspect-[2/3] overflow-hidden rounded-lg bg-surface-container-high">
                      ...
                      <?php if (!empty($a['puntuacion'])) { ?>
                        <span
                          class="anidex-score-badge absolute top-3 right-3 bg-surface-container-lowest/80 backdrop-blur px-2 py-1 rounded text-xs font-bold text-primary flex items-center gap-1">
          ...
          <div class="mt-12 flex justify-center hidden">
            <button
              ...
              type="button" aria-label="Cargar m&aacute;s resultados">
