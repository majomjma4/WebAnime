...
if ($statusFilter !== '' && strtoupper($statusFilter) !== 'ALL') {
    ...
    $normalizedStatus = strtoupper($statusFilter);
...
if ($typeFilter !== '' && strtoupper($typeFilter) !== 'ALL') {
...
$countStmt = $dbConn->prepare('SELECT COUNT(*) FROM anime' . $whereSql);
foreach ($params as $key => $value) {
    $countStmt->bindValue($key, $value);
}
$countStmt->execute();
$totalAnimes = (int) $countStmt->fetchColumn();
$totalPages = max(1, (int) ceil($totalAnimes / $perPage));
if ($page > $totalPages) {
    $page = $totalPages;
}
$offset = ($page - 1) * $perPage;

$listStmt = $dbConn->prepare('SELECT * FROM anime' . $whereSql . ' ORDER BY id DESC LIMIT :limit OFFSET :offset');
foreach ($params as $key => $value) {
...
$airingCount = (int) $dbConn->query("SELECT COUNT(*) FROM anime WHERE LOWER(estado) IN ('en emision', 'currently airing')")->fetchColumn();
...
<div class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm" data-admin-edit-modal>
  <div class="w-[min(720px,92vw)] rounded-3xl border border-outline-variant/20 bg-surface-container-low p-6 shadow-2xl">
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <label class="text-xs text-on-surface-variant">A̱o
