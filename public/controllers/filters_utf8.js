(function () {
      ...
      if (visible.length === 0 && q && q.length > 2) {
        if (!state.isFetchingGlobal) {
          ...
          if (emptyBox) {
            ...
            if (msg) msg.textContent = "Buscando en el catÃ¡logo global...";
          }
            ...
            .then(json => {
              ...
              if (json && json.data && json.data.length > 0) {
                ...
                if (emptyBox && emptyBox.querySelector("p")) {
                  emptyBox.querySelector("p").textContent = "No se encontraron resultados en el catÃ¡logo global.";
                }
              }
            })
            .catch(() => {
    ...
    function hydrateCardsWithResults(items, mediaType) {
          ...
          card.innerHTML = `
            <a class="block" href="#" aria-label="">
              <div class="relative aspect-[2/3] overflow-hidden rounded-lg bg-surface-container-high">
                ...
                <span data-card-year="1" class="absolute left-3 top-3 rounded-full bg-surface/80 px-3 py-1 text-xs font-bold text-on-surface"></span>
      ...
      if (isMoviesPage) {
        items.forEach((item, idx) => {
          ...
          if (badge) {
            badge.classList.remove("top-3");
            badge.classList.add("bottom-3", "px-4", "py-1.5", "text-sm");
          }
  ...
  function injectTestCards(isMoviesPage) {
    ...
    const movieAssignments = [
      { status: "En cartelera", type: "PelÃ­cula original" },
