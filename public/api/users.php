...
if ($action === 'list') {
    try {
        $stmt = $dbConn->query("
            SELECT
                u.id,
                u.codigo_publico AS public_id,
                u.nombre_mostrar AS name,
                u.correo AS email,
                u.creado_en,
                u.activo,
                u.bloqueado,
                u.motivo_bloqueo,
                u.penalizacion_hasta,
                u.es_premium,
                COALESCE(r.nombre, 'Registrado') AS base_role
            FROM usuarios u
            LEFT JOIN roles r ON r.id = u.rol_id
            WHERE LOWER(COALESCE(r.nombre, 'registrado')) <> 'admin'
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $users[] = [
                ...
                'role' => ((int) $row['es_premium'] === 1) ? 'Premium' : 'Registrado',
                'email' => $row['email'],
                'date' => date('d/m/Y', strtotime($row['creado_en'])),
                'status' => $isBlocked ? 'BLOQUEADO' : 'ACTIVO',
                'is_premium' => ((int) $row['es_premium'] === 1),
                'block_reason' => $row['motivo_bloqueo'] ?: '',
                'penalty_until' => $row['penalizacion_hasta'] ? date('d/m/Y H:i', strtotime($row['penalizacion_hasta'])) : null,
            ];
        }

        $statsStmt = $dbConn->query("
            SELECT
        ...
        echo json_encode([
            ...
            'stats' => [
                ...
                'premium_users' => (int) ($stats['premium_users'] ?? 0),
                'active_now' => (int) ($stats['active_now'] ?? 0),
            ],
        ]);
    } catch (Exception $e) {
